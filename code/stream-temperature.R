...
source("1-exposure/code/functions.R")
###############################################################################
...
lakes <- readRDS("1-exposure/data/spatial/layers/waterbodies_250.rds")# %>% st_crop(bounds)
rivers <- readRDS("1-exposure/data/spatial/layers/watercourse_250.rds") #%>% st_crop(bounds)

bounds0 <- c(xmin = -127, ymin = 49, xmax = -116.5, ymax = 56)
lakes0 <- readRDS("1-exposure/data/spatial/layers/waterbodies_lowRes.rds")# %>% st_crop(bounds)
rivers0 <- readRDS("1-exposure/data/spatial/layers/watercourse_lowRes.rds") #%>% st_crop(bounds)
BC <- readRDS("1-exposure/data/spatial/layers/BC_lowRes.rds")
...
source("1-exposure/code/load-PCIC-prelimFraser.R")
...
cu_list <- read.csv("~/Documents/PSF/PopulationAnalysis/conservationunits.csv") %>% subset(SPECIES_QUALIFIED == "SEL")
...
timing <- read.csv("1-exposure/data/timing/spawntiming_byCU.csv")
...
timing2 <- read.csv("1-exposure/data/timing/Life_cycle_table_20230327.csv") %>% subset(species == "SEL" & region == "Fraser")

# Add region to cu_list; focus on Fraser CUs with specific thermal tolerance data
cu_list$region <- timing$region[match(cu_list$cuid, timing$cuid)]
cu_list <- cu_list[which(cu_list$region == "Fraser"),]
...
zoi <- st_read(dsn = "1-exposure/data/spatial/ZOI/fraser-spawning-zoi/fraser_spawning_zoi_SEL.shp") %>% st_transform(crs = 4269)
...
cuids <- cu_list$cuid[match(cus_to_keep$culabel, cu_list$culabel)]

# Setup arrays to store output
streamTemp_dd <- array(NA,
            dim = c(length(cuids), 4, 3),
            dimnames = list(cus_to_keep$culabel, periods$period, c("median", "lower", "upper")))

# Setup a second matrix to look at difference when accounting for local topt
streamTemp_dd2 <- array(NA,
                       dim = c(length(cuids), 4, 3),
                       dimnames = list(cus_to_keep$culabel, periods$period, c("median", "lower", "upper")))

lowFlow_cmsd <- array(NA,
                 dim = c(length(cuids), 4, 3),
                 dimnames = list(cus_to_keep$culabel, periods$period, c("median", "lower", "upper")))

# Function to sum what's below and above thresh
degdays <- function(
